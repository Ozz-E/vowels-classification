...
# X_phonemes_1_2 = ...
X_phonemes_1_2 = np.zeros((np.sum(np.add(phoneme_id == 1, phoneme_id == 2)), 2))
...
# on f2 axis
# Then, classify each point [i.e., each (f1, f2) pair] of that grid, to either phoneme 1, or phoneme 2,
...
# Store these prediction in a 2D numpy array named "M", of shape N_f2 x N_f1 (the first dimension is f2
# so that we keep f2 in the vertical axis of the plot)
...
# initial grid for making predictions
...
for i in range(N_f2):
    ...
    for j in range(N_f1):
        ...
        prediction_2 = get_predictions(mu2, s2, p2, grid)
        if np.sum(prediction_1) > np.sum(prediction_2):
            M[i, j] = 0.0
        else:
            M[i, j] = 1.0
